# Contributing to vkaxcore-lib

We're working hard to make *vkaxcore-lib* the most powerful JavaScript library for working with Dash. Our goal is to have *vkaxcore-lib* be a library that can be used by anyone interested in Dash, and to level expertise differences with great design and documentation.

## Community

If there are any questions, etc., please feel to ask in one of the community channels:

- https://www.vkax.xyz/forum (Support Forum)
...
These are some global design goals in vkaxcore-lib that any change must adhere.
...
When possible, vkaxcore-lib objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of vkaxcore-lib methods. For example, calling `JSON.stringify` on an vkaxcore-lib object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'vkaxcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the vkaxcore object.

DO:
```javascript
var vkaxcore = require('../');
var PublicKey = vkaxcore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add vkaxevo git@github.com:vkaxevo/vkaxcore-lib.git
git pull --rebase vkaxevo master
```
...
```
Finally go to [github.com/vkaxevo/vkaxcore-lib](https://github.com/vkaxevo/vkaxcore-lib) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of vkaxcore-lib.

If you have any questions feel free to post them to
[github.com/vkaxevo/vkaxcore-lib/issues](https://github.com/vkaxevo/vkaxcore-lib/issues).
