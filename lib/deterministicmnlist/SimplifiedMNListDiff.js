...
SimplifiedMNListDiff.fromBuffer = function fromBuffer(buffer, network) {
  ...
  for (let i = 0; i < mnListSize; i += 1) {
    const mnListEntryLength = bufferReader.readVarintNum();

    data.mnList.push(
      SimplifiedMNListEntry.fromBuffer(
        bufferReader.read(mnListEntryLength),
        network
      )
    );
  }
...
SimplifiedMNListDiff.prototype.toBuffer = function toBuffer() {
  ...
  this.mnList.forEach((simplifiedMNListEntry) => {
    bufferWriter.writeVarintNum(simplifiedMNListEntry.toBuffer().length);
