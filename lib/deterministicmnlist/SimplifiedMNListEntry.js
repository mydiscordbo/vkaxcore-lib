...
SimplifiedMNListEntry.fromBuffer = function fromBuffer(buffer, network) {
  ...
  return SimplifiedMNListEntry.fromObject({
    ...
    payoutAddress: bufferReader.finished ? undefined : Address.fromPublicKeyHash(
      bufferReader.read(PUBKEY_ID_SIZE),
      network
    ).toString(),
    operatorPayoutAddress: bufferReader.finished ? undefined : Address.fromPublicKeyHash(
      bufferReader.read(PUBKEY_ID_SIZE),
      network
    ).toString(),
  });
...
SimplifiedMNListEntry.prototype.toBuffer = function toBuffer() {
  ...
  if (this.payoutAddress) {
    bufferWriter.write(
      Buffer.from(Address.fromString(this.payoutAddress).hashBuffer, 'hex')
    );
  }

  if (this.operatorPayoutAddress) {
    bufferWriter.write(
      Buffer.from(Address.fromString(this.operatorPayoutAddress).hashBuffer, 'hex')
    );
  }
